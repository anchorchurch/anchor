...
(function() {
  var Account, Map, i, links,
    __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };

  Account = (function() {

    function Account() {
      this.updateFormDate = __bind(this.updateFormDate, this);

      this.accountPage = __bind(this.accountPage, this);
      this.accountPage();
    }

    Account.prototype.accountPage = function() {
      var _this = this;
      if ($(".auto-submit").exists()) {
        $(".auto-submit").submit();
      }
      if ($(".form-date-fields").exists()) {
        $(".form-date-select").on("change focus", function() {
          return _this.updateFormDate();
        });
      }
      return $(".form-members-create-submit").on("click", function(event) {
        event.preventDefault();
        if ($(".birthday-date-year").val() !== "-1" && $(".birthday-date-month").val() !== "-1" && $(".birthday-date-day").val() !== "-1") {
          return $(".form-members-create").submit();
        } else {
          return alert("Please fill out the day, month, and year of your birthday.");
        }
      });
    };

    Account.prototype.updateFormDate = function() {
      return $(".form-birthday").val("" + ($(".birthday-date-year").val()) + "-" + ($(".birthday-date-month").val()) + "-" + ($(".birthday-date-day").val()));
    };
  ...
  Map = (function() {
    ...
    Map.prototype.map = function() {
      var latlng, styledMap, styles, _i, _len, _ref;
      styles = [
        {
          featureType: "all",
          elementType: "all",
      ...
      this.locations = [];
      this.latlngs = $(".map .locations .location");
      _ref = this.latlngs;
      ...
      return this.map.panToBounds(this.bounds);
    };

    Map.prototype.mark = function(latlng) {
      var infoWindow, infoWindowHtml, location, marker;
      ...
      return google.maps.event.addListener(marker, "click", function() {
        ...
        return this.map.panTo(marker.getPosition());
      });
    };
...
(function() {
  var Account, LiveStreaming, Maps,
    __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };
  ...
  Maps = (function() {
    ...
    Maps.prototype.mark = function(latlng) {
      var infoWindow, infoWindowHtml, location, marker;
      location = new google.maps.LatLng($(latlng).data("lat"), $(latlng).data("lng"));
      this.locations.push(location);
      marker = new google.maps.Marker({
    ...
    Maps.prototype.toggleBounce = function() {
      if (marker.getAnimation() !== null) {
        return marker.setAnimation(null);
      } else {
        return marker.setAnimation(google.maps.Animation.BOUNCE);
